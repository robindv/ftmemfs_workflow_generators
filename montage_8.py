...
class Montage(object):
    ...
    def __init__(self, filename):
        self.montage = ADAG("montage-8")
        self.n = 1
        self.input_workflow = ET.parse(filename).getroot()
        self.dependencies = {}

        self.mkdirs_job = Job(id="ID{:08d}".format(self.n), name="mkdir")
    ...
    def copy(self):
        ...
        for filename in ["big_region_20140107_113054_5873.hdr",
                         "statfile_20140107_113054_5873.tbl",
                         "pimages_20140107_113054_5873.tbl",
                         "cimages_20140107_113054_5873.tbl",
                        # "slist_20140107_113054_5873.tbl",
                         "region_20140107_113054_5873.hdr",
                        # "shrunken_20140107_113054_5873.hdr"
                         ]:
            self.n += 1
            target = File(filename)
            copyjob = Job(id="ID{:08d}".format(self.n), name="cp")

            infile = "/var/scratch/aua400/montage_8/{}".format(filename)

            if not os.path.isfile(infile):
                infile = infile.replace("_20140107_113054_5873","")

                if not os.path.isfile(infile):
                    print "Nop nop " + infile
        ...
        for job in self.input_workflow.getchildren():
            ...
            copyjob = Job(id="ID{:08d}".format(self.n), name="cp")
            copyjob.addArguments("/var/scratch/aua400/montage_8/{}".format(filename), target)
            copyjob.uses(target, link=Link.OUTPUT, transfer=True)
    ...
    def mProjectPP(self):
        for job in self.input_workflow.getchildren():
            ...
            self.n += 1
            mp = Job(id="ID{:08d}".format(self.n), name="mProjectPP")
    ...
    def mDiffFit(self):
        for job in self.input_workflow.getchildren():
            ...
            self.n += 1
            md = Job(id="ID{:08d}".format(self.n), name="mDiffFit")
    ...
    def mConcatFit(self):
        for job in self.input_workflow.getchildren():
            ...
            self.n += 1
            mc = Job(id="ID{:08d}".format(self.n), name="mConcatFit")
    ...
    def mBgModel(self):
        for job in self.input_workflow.getchildren():
            ...
            self.n += 1
            mb = Job(id="ID{:08d}".format(self.n), name="mBgModel")
    ...
    def mBackground(self):
        for job in self.input_workflow.getchildren():
            ...
            self.n += 1
            mb = Job(id="ID{:08d}".format(self.n), name="mBackground")
    ...
    def mFinish(self):
        ...
        for job in self.input_workflow.getchildren():
            ...
            self.n += 1
            mf = Job(id="ID{:08d}".format(self.n), name=job.attrib["name"])
...
montage = Montage("/var/scratch/aua400/montage_8/dag.xml")
montage.toFile("/home/rdevries/workflows/montage_8.dax")
